...
\begin{frame}{Inspection of predicted high frequencies}
  \centering
\only<1>{
    \includegraphics[width = .9\columnwidth]{solos_1141.png}

    Examples of predictions for the \textit{medley-solos-db} dataset. The proposed model handles correctly the harmonic structures.
  }
  \only<2>{
    \includegraphics[width = .9\columnwidth]{gtzan_1120.png}

    Examples of predictions for the \textit{gtzan} dataset. The proposed model handles correctly the average magnitude of complex spectral shapes.
    }
\end{frame}
